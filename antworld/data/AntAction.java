 ...
 * <br>
 * 
 * Each AntData contain a reference to an AntAction instance. The AntAction class is used by the client to tell the
 * server what action the corresponding ant asks to do. Each nest has a unique name. When a client opens a socket to the
 * server for the first time, it must send a nest name in CommData to request possession of a nest.
 * 
 * When the client's connection request is accepted, the server will return a CommData with an array of NestData objects
 * containing an element for each NestNameEnum below.
 */
...
public class AntAction implements Serializable
{
  ...
  /**
   * Every AntAction must have a type: MOVE | ATTACK | PICKUP | DROP | HEAL | ENTER_NEST | EXIT_NEST | BIRTH |DIED |
   * STASIS
   */
  public AntActionType type;

  /**
   * One of the 8 possible directions. The AntActionTypes requiring Direction are: MOVE, ATTACK, PICKUP, DROP, HEAL
   * (when executed by a medic ant outside of the nest)
   */
  public Direction direction;

  /**
   * (x,y) specify absolute coordinates in the Ant World grid. These fields are only used by the EXIT_NEST action. An
   * EXIT_NEST action is only valid when the specified coordinates are within a Manhattan Distance of
   * Constants.NEST_RADIUS from the ant's own nest center and, of course, when the ant is underground.
   */
  public int x, y;
  ...
  /**
   * Constructor that returns a deep copy of an AntAction object. It happens that all elements of AntAction are enums or
   * primitive types. Thus, a "deep copy" is actually the same as a copy; however, if this class were ever to be
   * expanded to include object instances, then this constructor would deep copy those objects.
   */
  ...
  /**
   * Used for debugging, this method returns a formatted string of this action and the field values required by the
   * value of type.
   */
