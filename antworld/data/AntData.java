...
public class AntData implements Comparable<AntData>, Serializable
{
  ...
  /**
   * Unique id. When the client uses the birth action to create an ant, the client should leave this set to the default
   * value of Constants.UNKNOWN_ANT_ID.<br>
   * <br>
   * When the server accepts an ant's birth action, the server sets the id to a unique value that monotonically
   * increases with the tick.
   */
  public int id = Constants.UNKNOWN_ANT_ID; // Use whenever you birth an ant

  /**
   * World Map pixel coordinates of this ant with (0,0) being upper-left. In the ant world map, each game object (an ant
   * or a food pile) occupies exactly one pixel. No two game objects may occupy the same pixel at the same time. NOTE:
   * food being carried by an ant is part of the ant game object.
   */
  ...
  /**
   * The type of food the ant is carrying. This equals null if the ant is not carrying any food.
   */
  public FoodType carryType = null;

  /**
   * The number of food units the ant is carrying. An ant may only carry one type of food at a time. The maximum number
   * of units an ant can carry is given by this.antType.getCarryCapacity()
   * */
  public int carryUnits = 0;

  /**
   * The client sets myAction to tell the server what the ant wants to do. The server returns this set to the action
   * that was actually taken.<br>
   * <br>
   * 
   * For example, the server set myAction to STASIS under two conditions:<br>
   * <ol>
   * <li>This ant attempts a move that is not legal (move onto water, attack in a direction where there is not an ant,
   * ...)</li>
  ...
  /**
   * The ticks until next action. This is used for actions that take more than one tick to complete. Currently, the only
   * such action is movement,
   * */
   ...
   * <ol>
   * <li>The ant can exit its nest any coordinates within its nest radius that is not occupied by a game object.</li>
   * <li>The ant is totally save: it cannot be attacked and never takes attrition damage.</li>
   * <li>The ant may heal itself this.antType.getHealPointsPerWaterUnit() health point by consuming one water drop from
   * the nest storage.</li>
   * <li>The ant may attack ants that are above ground in any of the 8 cells surrounding its underground coordinates.</li>
